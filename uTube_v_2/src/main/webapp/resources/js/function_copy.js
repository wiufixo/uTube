...
function btnEvent(){
		...
		const cmtId=$(this).prev().val();
			...
			const str = `
				...
				<div class="cmt-info">
					...
					<div class="info-top">
						<input class="cmtText my-2 form-control" type="text" value="${cmtText}">
					...
					$.ajax({
						  ...
						  success: function (re) {
							...
							input += `
		                        ...
		                        <span style="font-size: 0.9em; color: grey">${re.registDate}</span>
		                    </div>
		                    <div class="info-top">
		                        <span>
							`
		                    ...
		                    input += `<span class="cmtText my-2">${re.cmtContent}</span>
		                        </span>`;
	...
	$("#cmtInsertBtn").on("click", function(){
			...
			if($(this).next().length){ //버튼 뒤에 hidden 으로 답글을 달 댓글의 id 태그가 있는지 ==> 대댓글이다
	...
	$(document).on("click", "#cmtReplyBtn",  function(){
			...
			const info = $(this).parents(".cmt-info")[0];
			const cmtId = $(this).prev().val();
			$("#cmtInsertBtn").after(`<input type="hidden" name="cmtId" value="${cmtId}">`)
			$("#cmtContent").val("");
			$("#cmtContent").focus();
		})
	...
	$("#joinId").on("keyup", function() {
		...
		$.ajax({
			...
			success:function(re){
				...
				if(id.length < 5 || id.length >14){ //아이디 길이 제한 걸림
					$("#idNO").css("display", "block");
					if(re==1){ //아이디 중복 존재
						$("#idNO").css("display", "none");
		...
		function printCmtList(num){
					...
					$.ajax({
						  ...
						  success: function (re) {
						    ...
						    if (re.pagination.totalCnt == 0) {
						      list += `<li class="mb-2 text-center">등록된 댓글이 없습니다.</li>`;
						    } else {
						      list += `<li class="mb-2">댓글 총 ${re.pagination.totalCnt}개</li>`;
						      const arr = re.list;
						      for (cmt of arr) {
							            ...
							            for (var i = 0; i < cmtLevel; i++) {
							              list += "🔵";
						                    ...
						                    list += `<a href="${ctx}/member/detail?memId=${cmt.memId }"><span class="cmtWriter fw-bolder" style="font-size: 1.0em;">${cmt.memName}</span></a>
						                        ...
						                        <span style="font-size: 0.9em; color: grey">${cmt.registDate}</span>
						                    </div>
						                    <div class="info-top">
						                        <span>`;
						        if (cmtLevel != 0) {
						          ...
						          /* for(var i=0; i<cmtLevel; i++){
						                    ...
						                    list += `<span class="cmtText my-2">${cmt.cmtContent}</span>
						                        </span>`;
	    		        ...
	    		        if (item.postTitle.length > 30) {
	    		          ...
	    		          list += `${item.postTitle }`;
	    		                    ...
	    		                    list += `</span>
	    		                        ...
	    		                        <span class="item-desc">${item.memName }</span> <span class="item-desc">${item.hit }회</span> <span class="item-desc">${item.registDate}</span>
	    		                    </div>
			    ...
			    success: function (result){
			    	 let list = "";
				      for (item of result.list){
                          ...
                          list += `</div>
  		                    <div class="text"><span class="fw-bold">${item.memName }</span><br>조회수 ${item.hit }회<br>${item.registDate }</div>
  		                    </div>
  		                </div>
  		            </div>
  		      `;
...
function printFollowBtn(){
			$.ajax({
			    ...
			    success: function (re) {
			    	$("#followBtnBox").empty();
			    	if(loginMemId != 0 && loginMemId!=postMemId){
			    		if(re==1){
			    			$("#followBtnBox").append('<button id="followCancelBtn" class="subscribe fs-6">구독취소</button>');
			    		}else if(re==0){
				    		$("#followBtnBox").append('<button id="followBtn" class="subscribe fs-6">구독</button>');
			    		}
			    	}
			    }
			})
...
function checkAllToggle(){
	$("#allCheck").on("click", function() { //모두 체크표시 토글기능
...
function printDateBefore(){
			...
			for(var td of dateTd){
				...
				const str = elapsedTime(date);
				console.log(str);
				if(str.indexOf('일')>=0 || str.indexOf('달')>=0 || str.indexOf('월')>=0){
					return false;
				}
				$(td).html(str);
			}
